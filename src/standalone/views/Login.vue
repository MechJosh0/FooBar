	...
	<CenteredCard>
		...
		>
			...
			<q-input
				...
				type="password"
				filled
				:label="$t('views.login.form.fields.passwordVerify.label')"
				lazyRules
				...
				validation: {
					...
					passwordVerify: [
						(val) => // Required
						{
							...
							return this.$t('views.login.form.fields.passwordVerify.errors.required');
						},
						(val) => // Must match
						{
							if(val === this.password) return true;

							return this.$t('views.login.form.fields.passwordVerify.errors.noMatch');
						}
					]
				},
				password: '',
